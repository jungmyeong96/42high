...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_calc_sprite2_bonus.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: junghan <junghan@student.42seoul.kr>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/02 02:00:46 by junghan           #+#    #+#             */
/*   Updated: 2021/04/02 02:00:50 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_color_in_spbuf(t_sprite *sprite, t_info *info, int x, int tex)
{
	...
	while (++y < sprite->drawend_y)
	{
		...
		if ((tex == 10 || tex == 15 || tex == 14) && info->skill_t != 0
				&& x == info->win_wid / 2 && y == info->win_hei * 6 / 10
				&& sprite->color != -16777216)
		{
			info->sp_hit = 1;
			if (info->skill1 == 1 && info->skill_t == 1)
				sprite->save_mob = sprite->one_mob;
		}
