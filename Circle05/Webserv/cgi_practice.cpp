...
int main(int argc, char *argv[]) {
  if (argc == 5) {
    ...
    command = setCommand(argv[1], argv[3]); //cgi_tester, GET
    ...
    if (!strcmp(command[0], "php-cgi")) {
      env_set["SCRIPT_NAME"] = "/Users/junghan/Desktop/DreamXWebserv/tester/php-cgi";
    ...
    */
    ...
    if (!pid) {
      ...
      close(pipe_fd[1]);
      if (!strcmp(command[0], "php-cgi"))
        execve("/Users/junghan/Desktop/DreamXWebserv/tester/php-cgi", command, environ);
      // personal mac -> /opt/homebrew/bin/php-cgi
      // cluster mac  -> 1) install php on host
      //              -> 2) /Users/$(USER)/.brew/bin/php-cgi
      else if (!strcmp(command[0], "cgi_tester"))
        execve("./tester/cgi_tester", command, environ);
      // exit(1);
    } 
    else {
      // close(pipe_fd[1]);
