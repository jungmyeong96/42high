...
void fill_response(char *header, int status, long len, std::string type, char *body) {
	...
	sprintf(header, RESPONSE_FMT, status, status_text, len, body);
...
//환경변수 설정
	...
	while (1)
	{
		...
		cout << "총 이벤트의 수 new event = " << new_events << endl;
		for (int i = 0; i < new_events; ++i)
		{
			...
			else if (curr_event->filter == EVFILT_READ)
			{
				...
				else if (clients.find(curr_event->ident)!= clients.end())
				{
					...
					else
					{
						...
						header->uri = strrchr(header->uri, '/');
						header->version = strtok(NULL, "\n");



							env_set["QUERY_STRING"] = "color=black&sport=123";
							env_set["REQUEST_METHOD"] = header->method;
							env_set["REDIRECT_STATUS"] = "CGI";
							env_set["SCRIPT_FILENAME"] = header->uri + 1;
							env_set["SERVER_PROTOCOL"] = header->version;
							env_set["PATH_INFO"] = header->uri + 1;
							//env_set["CONTENT_TYPE"] = "application/x-www-form-urlencoded";
							env_set["CONTENT_TYPE"] = "text/plain";
							env_set["GATEWAY_INTERFACE"] = "CGI/1.1";
							env_set["PATH_TRANSLATED"] = header->uri;
							env_set["REMOTE_ADDR"] = "127.0.0.1";
							env_set["REQUEST_URI"] = header->uri;
							env_set["SERVER_PORT"] = "8020";
							env_set["SERVER_PROTOCOL"] = header->version;
			...
			else if (curr_event->filter == EVFILT_WRITE)
			{
				...
				if (!strcmp(header->method, "GET"))
				{
					...
					if (header->cgi == 1)
					{
						...
						char **command1 = setCommand("php-cgi", header->uri + 1);
						...
						else
						{
							...
							write(header->fd, r_header, strlen(r_header)); //현재 fd 5
							printf("*************7\n");
							wait(NULL);
							printf("*************8\n");
