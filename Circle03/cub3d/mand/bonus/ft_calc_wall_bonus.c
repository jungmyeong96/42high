...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_calc_wall_bonus.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: junghan <junghan@student.42seoul.kr>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/02 02:02:29 by junghan           #+#    #+#             */
/*   Updated: 2021/04/02 02:02:32 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d_bonus.h"

void	wallpoint(t_wall *wall, t_info *info)
{
	wall->line_hei = (int)(info->win_hei / wall->perpwall_d);
	wall->drawstart = -wall->line_hei / 2 + info->win_hei / 2
		+ info->pitch + (info->pos_z / wall->perpwall_d);
	if (wall->drawstart < 0)
		wall->drawstart = 0;
...
void	work_dda(t_wall *wall, t_info *info)
{
	while (wall->hit == 0)
	{
	...
	if (wall->perpwall_d <= 0.05)
		wall->perpwall_d = 0.05;
}
...
void	set_sidedist(t_wall *wall, t_info *info)
{
	if (wall->raydir_x < 0)
	{
		wall->step_x = -1;
...
void	calc_wall(t_info *info, int x)
{
	...
	info->win_mid = info->win_hei * 0.08;
	set_wall_info(&wall, info, x);
	set_sidedist(&wall, info);
	work_dda(&wall, info);
	wallpoint(&wall, info);
